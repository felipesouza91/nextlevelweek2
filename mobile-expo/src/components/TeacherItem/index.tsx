...
const TeacherItem: React.FC<TeacherItemProps> = ({ value, favorited }) => {
  const [isFavorited, setIsFavorited] = useState(favorited);

  function handleLinkToWhatsApp() {
    Linking.openURL(`whatsapp://send?phone=${value.whatsapp}`)
    api.post('/connections', {user_id: value.user_id})
  }
  ...
  async function handleToggleFavorites() {
    ...
    if (isFavorited) {
      const favoriteIndex = favoritesArray
        .findIndex((teacherItem: Teacher ) => teacherItem.id === value.id )
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.buttonsContainer}>
          <RectButton 
            ...
            { isFavorited ? <Image source={unfavorteIcon} /> :
